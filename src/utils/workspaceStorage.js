...
const STORAGE_KEYS = {
  WORKSPACE_SETTINGS: StorageKeys.get('neurosensefx-workspace-settings'),
  DISPLAY_SIZES: StorageKeys.get('neurosensefx-display-sizes')
};
...
export function saveWorkspaceSettings(settings) {
  try {
    EnvironmentStorage.setItem('neurosensefx-workspace-settings', JSON.stringify(settings));
  } catch (error) {
...
export function loadWorkspaceSettings() {
  try {
    const stored = EnvironmentStorage.getItem('neurosensefx-workspace-settings');
    if (stored) {
...
export function saveDisplaySizes(displays) {
  try {
    ...
    EnvironmentStorage.setItem('neurosensefx-display-sizes', JSON.stringify(sizes));
  } catch (error) {
...
export function loadDisplaySizes() {
  try {
    const stored = EnvironmentStorage.getItem('neurosensefx-display-sizes');
    if (stored) {
...
export function clearWorkspaceData() {
  try {
    EnvironmentStorage.removeItem('neurosensefx-workspace-settings');
    EnvironmentStorage.removeItem('neurosensefx-display-sizes');
  } catch (error) {
...
export function getStorageInfo() {
  try {
    const workspaceSettings = EnvironmentStorage.getItem('neurosensefx-workspace-settings');
    const displaySizes = EnvironmentStorage.getItem('neurosensefx-display-sizes');
...
export function saveWorkspaceLayout(layout) {
  try {
    ...
    EnvironmentStorage.setItem('neurosensefx-workspace-layout', data);
  } catch (error) {
...
export function saveGlobalConfigState(config) {
  try {
    ...
    EnvironmentStorage.setItem('neurosensefx-global-config', data);
  } catch (error) {
...
export function saveUserPreferences(preferences) {
  try {
    EnvironmentStorage.setItem('neurosensefx-user-preferences', JSON.stringify(preferences));
...
export function loadUserPreferences() {
  try {
    const stored = EnvironmentStorage.getItem('neurosensefx-user-preferences');
    if (stored) {
      return JSON.parse(stored);
    }
  } catch (error) {
...
export function getComprehensiveStorageInfo() {
  try {
    ...
    // Check new storage
    const workspaceLayout = EnvironmentStorage.getItem('neurosensefx-workspace-layout');
    const globalConfig = EnvironmentStorage.getItem('neurosensefx-global-config');
    const userPreferences = EnvironmentStorage.getItem('neurosensefx-user-preferences');
    const workspaceMetadata = EnvironmentStorage.getItem('neurosensefx-workspace-metadata');
...
export function validateStorageIntegrity() {
  try {
    ...
    const layoutData = EnvironmentStorage.getItem('neurosensefx-workspace-layout');
    if (layoutData) {
      try {
    ...
    // Check global config
    const configData = EnvironmentStorage.getItem('neurosensefx-global-config');
    if (configData) {
      try {
...
export function migrateStorageFormat() {
  try {
    ...
    const newLayoutExists = EnvironmentStorage.getItem('neurosensefx-workspace-layout');
    if (newLayoutExists) {
