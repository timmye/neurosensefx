...
| ---- | ---- | ------------ |
| `auth-flow.spec.js` | Authentication flow E2E tests (login, register, session) | Testing authentication workflow |
...
| `debug-auth.spec.js` | Auth debug tests for troubleshooting | Debugging authentication issues |
| `fj-api-test.spec.js` | FinancialJuice FJWidgets API investigation test | Debugging FinancialJuice integration |
...
| `server-persistence.spec.js` | Server-side persistence E2E tests (workspace import/export, drawing sync) | Testing server persistence features |
