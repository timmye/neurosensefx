...
| ---- | ---- | ------------ |
| `chartConfig.js` | Resolution/window constants, barSpace mappings, TRANSITION_MATRIX (11-window tick level definitions), cTrader period range limits, cache caps. RESOLUTION_FLOOR and TICK_INTERVALS are legacy exports with no runtime consumers | Adding resolutions, modifying bar spacing, changing transition matrix levels, changing fetch limits |
| `xAxisCustom.js` | Custom calendar-aware x-axis via registerXAxis — reads TRANSITION_MATRIX from chartConfig, uses module-state `setAxisWindow()` for window updates, rank-priority MIN_FLOOR collision suppression. Exports: setAxisChart, setAxisWindow, setAxisResolution, snapToBar, formatBoundaryLabel, generateTicks | Modifying x-axis behavior, adjusting tick density, changing label formats |
