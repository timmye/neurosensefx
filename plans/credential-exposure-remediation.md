...
>
> **Progress (2026-06-25):** **RESOLVED.** Root `.env` fully on app #2 (`client_id` `15125_…` + fresh `access_token`/`refresh_token`, account `38998989` — all gitignored/untracked, never committed); `.idx/dev.nix` app #1 secret stripped → placeholders; secret-free OAuth helper `scripts/ctrader-token-exchange.cjs` added (only untracked new file); `.env.bak` deleted. **Verified live:** dev backend `:8080` `GET /health` → `ctrader CONNECTED`, live cTrader ticks flowing (single stable backend). **App #1 DELETED in portal 2026-06-25 = revocation complete — exposure remediated at source.** Phase 5 history scrub **intentionally skipped** — app #1's secret/tokens in git history are now inert (app deleted); user judged the force-push churn not worth it. Residual (accepted): app #2 secret+tokens were pasted into the chat transcript; clean in repo. Token auto-refresh not yet exercised (fresh token authed directly). Tracker: `plans/tidiness-remediation.md`.
