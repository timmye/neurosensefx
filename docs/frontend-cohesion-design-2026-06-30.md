...
**Carried to Phase 2 / Option 3 (still deferred — do not re-flag):** chart chrome on tokens (ChartToolbar / ChartDisplay frame / OverlayContextMenu / DisplayCanvas); z-index unification; PriceTicker → `<DisplayFrame>` wrap-up (incl. tokenizing its raw-rgba close/refresh buttons); light-theme polish on a few residual `rgba` overlays/glows; atom library + 3-tier component tokens.
