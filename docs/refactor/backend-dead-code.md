...
Tier 3 #13 — logging layer, **Phase 1 done**: added a dependency-free `utils/Logger.js` (`createLogger(module)` → `{debug,info,warn,error}`, threshold from `config.logLevel` / env `LOG_LEVEL`, delegates to `console`, ANSI color in non-prod) and migrated the five noise-source files — `MarketProfileService.js` (retired the `DEBUG_PROFILE` gate; use `LOG_LEVEL=debug`), `WebSocketServer.js`, `server.js`, `DataRouter.js`, `utils/SafeSender.js`. Per-client connect/disconnect + heartbeat summary now log at `debug`; startup/session lifecycle at `info`. 83 unit tests still pass. **Phase 2** (the remaining quieter files: CTraderSession, TradingViewSession, RequestCoordinator, authRoutes, persistenceRoutes, etc.) remains open, as do Tier 2 #11–#12.
