...
| `frontend-cohesion-design-2026-06-30.md` | Frontend cohesion/design-system design doc — token layer, color role map, `<DisplayFrame>`/`<IconButton>`/`<AddMenu>` architecture, display-type semantics. Reference for the implemented design system | Modifying tokens, display primitives, or the add menu |
| `symbol-subscription-failure-modes-2026-07-01.md` | Symbol subscription failure modes & trader-feedback intelligence — cTrader vs TradingView failure matrix (with proposed trader messages), the wire/renderer gaps behind "waiting for market data", trader-message vocabulary, 3-layer fix path | Planning symbol-entry UX/validation, debugging subscription failures, reconciling cTrader vs TradingView behavior |
