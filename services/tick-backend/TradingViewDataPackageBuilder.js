...
class TradingViewDataPackageBuilder {
    ...
    buildDataPackage(symbol, todaysOpen, adr, todaysM1Candles, previousDay, lastCandle, estimatePipDataFn) {
        ...
        return {
            ...
            symbol,
            open: todaysOpen,
            high: todaysM1Candles.length > 0 ? Math.max(...todaysM1Candles.map(c => c.high)) : lastCandle.high,
            low: todaysM1Candles.length > 0 ? Math.min(...todaysM1Candles.map(c => c.low)) : lastCandle.low,
