...
| `layer/` | Phase-0 characterization unit tests for the **built** cTrader-layer pure-logic modules (`CTraderCommandMap`, `CTraderCommand`, `CTraderProtobufReader`, `CTraderEncoderDecoder`); pins today's behavior incl. the L2 byte-derivation proof and the L3/L8/L9/L10 defects as the baseline for the layer-hardening fixes | Adding layer unit coverage; proving the L-fixes are non-regressing |
| `integration/` | Phase-0 offline integration tests that instantiate the REAL `CTraderConnection` against a scriptable mock cTrader TLS server (`helpers/mockCtraderServer.js`); pins L1/L2/L3/L5 connect/heartbeat/command behavior | Verifying real connection/socket/command behavior offline (mock server generates its self-signed cert via `openssl`, which must be on PATH) |
