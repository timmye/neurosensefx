...
| `CTraderTransportAdapter.js` | Transport tier over the read-only cTrader-Layer library; per-RPC TTL + reject-on-close around `sendCommand`; idempotent `open()`; raw clientMsgId-free `ProtoHeartbeatEvent` keepalive written to the captured TLS socket | Connection lifecycle, transport/TTL, the raw-heartbeat seam, debugging the cTrader reconnect loop |
